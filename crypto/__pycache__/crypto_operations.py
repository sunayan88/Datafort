...
def decrypt_file_for_restore(encrypted_data, encrypted_symmetric_key, iv, tag, system_private_key):
    ...
    then decrypt the data. Raises InvalidTag if authentication fails.
    """
    # Decrypt symmetric key with system private key
    ...
    try:
        ...
        # Authentication failed – data has been tampered with or tag is wrong
        raise InvalidTag("Backup integrity check failed – data may have been tampered with") from e


# ---------- Approval Signature Verification ----------
def verify_approval_signature(public_key, request_id, signature):
    """Verify that the signature matches the request ID."""
    data = f"approve:{request_id}".encode()
    try:
        public_key.verify(
            signature,
            data,
